...
api.interceptors.response.use(
  ...
  (err) => {
    const msg = err.response?.data?.message || err.response?.data?.data?.message || err.message || 'Something went wrong'
...
export const authApi = {
  ...
  register: (data: { name: string; email: string; mobile: string; password: string }) => api.post('/auth/register', data),
}
...
export const couponsApi = {
  ...
  validate: (code: string, amount: number) => api.post('/coupons/validate', { code, orderAmount: amount }),
